...
public class Client implements ClientCom {
  ...
  // for debug use
  public ErrorType send2ServerDebeg(String message2send) {
    return sendPost(message2send, true);
  }

  private ErrorType sendPost(String message2send, boolean debugMode) {// sends a
                                                                      // message
                                                                      // to
                                                                      // server
    ...
    try {
      ...
      if(debugMode)
        System.out.println("an ERROR acurred while creating URL");// for
                                                                  // debugging
                                                                  // purpose
    ...
    try {
      ...
      if(debugMode)
        System.out.println("an ERROR acurred while connecting to server");// for
                                                                          // debugging
                                                                          // purpose
      return ErrorType.SEND_FAIL;
    }
    ...
    try {
      ...
      if(debugMode)
        System.out
            .println("an ERROR acurred while tring to get response from server");// for
                                                                                 // debugging
                                                                                 // purpose
      return ErrorType.SEND_FAIL;
    }
    ...
    if(debugMode) {// / if we are in debug mode we would like to read the server
                   // response
      String inputLine;
      ...
      try {
        ...
        System.out.println("could not close DataOutputStream");
      }
